...
def produce_hf_anno(num_fields: int = 2, anno: pd.DataFrame = None) -> pd.DataFrame:
    ...
    field_filenames = list(map(lambda l: [f"{l[0]}_{i}.png" for i in range(num_fields)], plates_wells_all))
    filename_plate_well_df = pd.DataFrame({"file_name": flatten_extend(field_filenames), "Plate_Well": [filename[:-6] for filename in flatten_extend(field_filenames)]})
    huggingface_df = pd.merge(filename_plate_well_df, anno, on="Plate_Well")
    return huggingface_df


def save_hf_anno(huggingface_df: pd.DataFrame, converted_data_path: str, split_folder: str = "all", mixed_data_type_columns: Optional[List[str]] = None) -> None:
